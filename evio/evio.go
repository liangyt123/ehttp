...
type EventsFunc interface {
	Serving(server Server) (action Action)
	// Opened fires when a new connection has opened.
	// The info parameter has information about the connection such as
	// it's local and remote address.
	// Use the out return value to write data to the connection.
	// The opts return value is used to set connection options.
	Opened(c Conn) (out []byte, opts Options, action Action)
	// Closed fires when a connection has closed.
	// The err parameter is the last known connection error.
	Closed(c Conn, err error) (action Action)
	...
	Detached(c Conn, rwc io.ReadWriteCloser) (action Action)
	// PreWrite fires just before any data is written to any client socket.
	PreWrite()
	// Data fires when a connection sends the server data.
	// The in parameter is the incoming data.
	// Use the out return value to write data to the connection.
	Data(c Conn, in []byte) (out []byte, action Action)
...
func (e Events) Serving(server Server) (action Action) {
	return
}
...
// Use the out return value to write data to the connection.
// The opts return value is used to set connection options.
func (e Events) Opened(c Conn) (out []byte, opts Options, action Action) {
	return
}

// Closed fires when a connection has closed.
// The err parameter is the last known connection error.
func (e Events) Closed(c Conn, err error) (action Action) {
	return
}
...
// underlying socket connection. It can be freely used in goroutines
// and should be closed when it's no longer needed.
func (e Events) Detached(c Conn, rwc io.ReadWriteCloser) (action Action) {
	return
}

// PreWrite fires just before any data is written to any client socket.
func (e Events) PreWrite() {
	return
}
...
// The in parameter is the incoming data.
// Use the out return value to write data to the connection.
func (e Events) Data(c Conn, in []byte) (out []byte, action Action) {
	return
}

// Tick fires immediately after the server starts and will fire again
// following the duration specified by the delay return value.
func (e Events) Tick() (delay time.Duration, action Action) {
	return
}
