...
func (es echoServer) Serving(srv evio.Server) (action evio.Action) {
...
func (es echoServer) Opened(c evio.Conn) (out []byte, opts evio.Options, action evio.Action) {
	c.SetContext(&evio.InputStream{})
	//log.Printf("opened: laddr: %v: raddr: %v", c.LocalAddr(), c.RemoteAddr())
	return
}

func (es echoServer) Closed(c evio.Conn, err error) (action evio.Action) {
	//log.Printf("closed: %s: %s", c.LocalAddr().String(), c.RemoteAddr().String())
	return
}

func (es echoServer) Data(c evio.Conn, in []byte) (out []byte, action evio.Action) {
	if in == nil {
		return
	}
...
func main() {
	...
	// Start serving!
	evio.ListenAndServe(addrs[0], events)
